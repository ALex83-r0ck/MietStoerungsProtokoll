...
def insert_laermdaten(datum: str, beginn: str, ende: str, grund: str, verursacher: str, auswirkung: int) -> None:
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("INSERT INTO laermdaten (datum, beginn, ende, grund, verursacher, auswirkung) VALUES (?, ?, ?, ?, ?, ?)",
...
def insert_massnahmen(datum: str, massnahme: str, ergebnis: str) -> None:
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("INSERT INTO massnahmen (datum, massnahme, ergebnis) VALUES (?, ?, ?)",
