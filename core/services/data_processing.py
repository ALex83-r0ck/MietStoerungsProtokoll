...
def get_all_data(force_reload: bool = False) -> pd.DataFrame:
    ...
    df['beginn'] = pd.to_datetime(df['datum'].dt.strftime('%Y-%m-%d') + ' ' + df['beginn'].astype(str))
...
def plot_histogramm_dauer(df: pd.DataFrame) -> None:
    if df.empty or 'dauer' not in df.columns: return
    dauer = df['dauer'].dropna()
    if dauer.empty: return
    ...
    if patches:
        ...
        plt.text(
            center, height + max(heights) * 0.03,
            f"Häufigste Dauer\n{int(center)} Min",
            ha='center', va='bottom', fontsize=12, fontweight='bold',
            color='red', bbox=dict(facecolor='white', alpha=0.9, edgecolor='red', boxstyle='round')
        )
...
def plot_top_stoerungen(df: pd.DataFrame) -> None:
    if df.empty or 'grund' not in df.columns: return
...
def plot_uhrzeiten(df: pd.DataFrame) -> None:
    if df.empty or 'beginn' not in df.columns: return
    stunden = df['beginn'].dt.hour.value_counts().sort_index()
    if stunden.empty: return
...
def plot_prognose(df: pd.DataFrame, tage: int = 14) -> None:
    ...
    plt.figure(figsize=(11, 6))
    sns.lineplot(data=df_temp.tail(30), x='datum', y='dauer', label='Vergangenheit', color='gray', linewidth=2)
    sns.lineplot(x=zukunft_daten, y=vorhersage.flatten(), label=f'Prognose (+{tage} Tage)', color='#d62728', linewidth=3)

    plt.title(f"Prognose: Störungsdauer nächste {tage} Tage", fontsize=16, pad=20, fontweight='bold')
    plt.xlabel("Datum")
    plt.ylabel("Dauer (Minuten)")
    plt.legend()
    plt.grid(alpha=0.3)
